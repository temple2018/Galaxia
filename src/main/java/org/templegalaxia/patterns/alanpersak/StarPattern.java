...
/**
 * An animation of randomized stars. A star is defined by its "center" pixel, size in pixels,
 * brightness and lifetime. Each star occupies a number of pixels equal to its size, plus one empty
 * pixel on either side, and stars cannot overlap. Stars are created with randomized parameters in
 * random locations and fade in and out over the course of their lifetimes.
 */
@LXCategory("Alan Persak")
public class StarPattern extends TemplePattern {

  private BoundedParameter timeScaleParam =
      new BoundedParameter("Time Scale", 1, .1, 20)
          .setDescription("Scales the speed of animation.");
  private BoundedParameter rateParam =
      new BoundedParameter("Rate", 6, 1, 50)
          .setDescription(
              "The probability of a star being created. A rate of 10 means on average 10 stars per second when no stars are present. The actual rate will be less since stars cannot overlap.");
  private BoundedParameter minSizeParam =
      new BoundedParameter("Minimum Size", 1, 1, 10)
          .setDescription("The minimum size of a star in pixels.");
  private BoundedParameter maxSizeParam =
      new BoundedParameter("Maximum Size", 3, 1, 10)
          .setDescription("The maximum size of a star in pixels.");
  private BoundedParameter minBrightnessParam =
      new BoundedParameter("Minimum Brightness", 40, 0, 100);
  private BoundedParameter maxBrightnessParam =
      new BoundedParameter("Maximum Brightness", 100, 0, 100);
  private BoundedParameter brightnessStepsParam =
      new BoundedParameter("Brightness Steps", 3, 1, 10)
          .setDescription("The number of brightness steps.");
  private BoundedParameter minLifespanParam =
      new BoundedParameter("Minimum Lifespan", 60, 1, 1000)
          .setDescription("The minimum duration of randomized star lifetimes.");
  private BoundedParameter maxLifespanParam =
      new BoundedParameter("Maximum Lifespan", 300, 1, 1000)
          .setDescription("The maximum duration of randomized star lifetimes.");
  private BoundedParameter fadeTimeParam =
      new BoundedParameter("Fade Time", 30, 1, 300)
          .setDescription("The amount of time for a star to fade in and out.");
  private BoundedParameter baseBrightnessParam =
      new BoundedParameter("Base Brightness", 0, 0, 100)
  ...
  private HashSet<Integer> usedPoints =
      new HashSet<
          Integer>(); // Set of indicies of used pixels. Stars cannot occupy pixels that are already
  // used.
  ...
  /**
   * Adds points to the set of used points only if they are all unused. Returns whether or not they
   * were added.
   */
  ...
  private static double getBrightness(
      double brightness, double baseBrightness, double lifespan, double fadeTime, double age) {
    if (age < fadeTime) {
  ...
  /**
   * Returns whether a star should be created, based on parameters for the probabilistic rate of
   * creation.
   */
  private static boolean shouldCreateStar(
      Random random, double rate, int numPixels, double deltaMs) {
  ...
  private class Star {
    ...
    public int
        numLitPixels; // Number of lit pixels, which may be less than size when it goes out of
    // bounds.
